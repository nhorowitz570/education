...
const BEAT: Record<string, string> = {
  ...
  situation: `Task: a realistic situation that makes the idea matter: the session's opening, or a bridge into the next idea (see this step's purpose). 60–120 words, specific numbers and people, fictional but plausible. Show the key figures in a visual (stat, bar or statement) when there are quantities. End with the tension, not a question — the explanation comes next. If the learner is new to the idea, use no term they haven't been taught. Record the scenario facts you invent in scenario_facts so later steps stay consistent.`,
