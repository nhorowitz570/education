...
- **Email template → Magic link:** paste `supabase/templates/magic_link.html`. It sends both a link to `/auth/confirm` and a 6-digit code.
...
2. `vercel.json` schedules `/api/cron` every 15 minutes (reminders, photo expiry, stale voice-call cleanup).
